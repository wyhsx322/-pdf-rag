...
复用现有评估习惯（eval_retrieval.py 的 run_eval、eval_prompts.py 的落盘），
结果打印为对比表并落盘 output/eval_trajectory_{timestamp}.json。

用法：
  uv run python -m evaluations.eval_trajectory            # 跑全部黄金任务
  uv run python -m evaluations.eval_trajectory --limit 1  # 只跑第 1 个（快速冒烟）
...
from server.core.config import PROJECT_ROOT, DASHSCOPE_API_KEY_ENV
