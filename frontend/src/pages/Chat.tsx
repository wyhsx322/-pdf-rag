...
export default function Chat() {
  ...
  // ── 输入框（空态居中 / 会话态底部共用） ──
  const inputBar = (
    <div className="rounded-2xl border border-slate-200 bg-white p-2 shadow-soft transition-shadow focus-within:shadow-glow">
      <textarea
        ref={taRef}
        rows={1}
        value={input}
        ...
        className="max-h-[200px] w-full resize-none bg-transparent px-3 py-2 text-sm text-slate-800 outline-none placeholder:text-slate-400"
      />
      <div className="flex items-center justify-between px-1.5 pb-0.5">
        ...
        {loading ? (
          ...
          <button
            onClick={handleSend}
            disabled={!input.trim()}
            className="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-r from-indigo-500 to-violet-500 text-white shadow-soft transition-all hover:shadow-glow disabled:opacity-40 disabled:shadow-none"
          >
  ...
  return (
    <div className="flex h-full min-h-0">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        {/* 顶部工具条 */}
        <div className="flex h-14 shrink-0 items-center gap-3 border-b border-slate-200 bg-white/80 px-5 backdrop-blur">
          <Select
        ...
        {isEmpty ? (
          ...
          <div className="flex flex-1 flex-col items-center justify-center px-6">
            <div className="w-full max-w-2xl text-center">
              ...
              <h1 className="text-2xl font-semibold text-slate-800">今天想研究什么？</h1>
              <p className="mt-2 text-sm text-slate-400">
                选择知识库后提问，多智能体将基于论文内容作答并回链引用来源
              </p>
              <div className="mt-7">{inputBar}</div>
              <div className="mt-4 flex flex-wrap justify-center gap-2">
                {SUGGESTIONS.map(q => (
                  <button
                    key={q}
                    onClick={() => { setInput(q); taRef.current?.focus() }}
                    className="rounded-full border border-slate-200 bg-white px-3.5 py-1.5 text-xs text-slate-500 transition-colors hover:border-indigo-300 hover:text-indigo-600"
                  >
                    {q}
                  </button>
                ))}
