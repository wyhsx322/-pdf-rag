...
import { Check, FileText, Loader2, Play, Database, Trash2, X } from 'lucide-react'
import type { Document } from '../../entities/knowledge-base/model'
import { DOC_STATUS_LABELS } from '../../entities/knowledge-base/model'
import type { BatchProgressEvent } from '../../entities/knowledge-base/api'
...
export default function DocumentRow({
  doc, onDelete, onProcess, onViewVectorStats, onDeleteVectors, onReindex, processing,
  batchSelected, batchProgress, batchDisabled, onBatchToggle,
}: Props) {
      ...
      {onBatchToggle && (
        <td className="px-4 py-3">
          {batchProgress?.status === 'processing' ? (
            <span className="inline-flex items-center gap-1.5 text-xs text-indigo-600"><Loader2 className="h-3.5 w-3.5 animate-spin" />{batchProgress.message || '处理中'}</span>
          ) : batchProgress?.status === 'done' ? (
            <span className="inline-flex items-center gap-1.5 text-xs text-emerald-600"><Check className="h-3.5 w-3.5" />完成</span>
          ) : batchProgress?.status === 'error' ? (
            <span className="inline-flex items-center gap-1.5 text-xs text-rose-600"><X className="h-3.5 w-3.5" />{batchProgress.message || '异常'}</span>
          ) : (
            <span className="text-xs text-slate-300">待选择</span>
          )}
        </td>
      )}
