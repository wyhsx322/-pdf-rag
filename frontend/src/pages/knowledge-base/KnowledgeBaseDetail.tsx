...
import { ArrowLeft, Search, FolderInput, Trash2, Zap } from 'lucide-react'
import { toast } from 'sonner'
import type { KnowledgeBaseDetail as KBDetail, ChunkMethod, ChunkResult } from '../../entities/knowledge-base/model'
import {
...
} from '../../entities/knowledge-base/api'
import type { VectorStats, BatchProgressEvent } from '../../entities/knowledge-base/api'
import { KB_TYPE_LABELS, CHUNK_METHOD_LABELS, DEFAULT_SEPARATORS, DOC_STATUS_LABELS } from '../../entities/knowledge-base/model'
import UploadZone from './UploadZone'
import DocumentRow from './DocumentRow'
import SeparatorEditor from './SeparatorEditor'
import ChunkPreview from './ChunkPreview'
import { Card, Button, Badge, Input, Select, Switch, Slider, Spinner, Dialog, Tabs } from '../../shared/ui'
    ...
    <div className="h-full overflow-y-auto">
      <div className="mx-auto max-w-7xl px-10 py-9">
        ...
        <div className="mb-7 flex flex-wrap items-center gap-4">
          <button onClick={() => navigate('/knowledge')} className="rounded-xl border border-white/70 bg-white/70 p-2.5 text-slate-400 shadow-soft transition-colors hover:text-slate-700">
            <ArrowLeft className="h-5 w-5" />
          </button>
          <div className="min-w-0">
            <p className="mb-1 text-xs font-semibold uppercase tracking-[0.22em] text-indigo-500">Knowledge Base</p>
            <h2 className="flex items-center gap-2.5 text-3xl font-semibold tracking-tight text-slate-950">
              <span className="truncate">{kb.name}</span>
              <Badge tone="indigo">{KB_TYPE_LABELS[kb.type]}</Badge>
            </h2>
            {kb.description && <p className="mt-1 truncate text-sm text-slate-500">{kb.description}</p>}
          </div>
            ...
            <Card className="overflow-hidden">
              <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-100 px-6 py-4">
                <div>
                  ...
                  <p className="mt-1 text-xs text-slate-400">PDF 解析 → 图片摘要 → 切片 → 向量化入库</p>
                </div>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {kb.documents.length > 0 && (
                    batchRunning ? (
                      <Button size="sm" variant="danger" onClick={() => { batchCtl?.abort(); setBatchRunning(false); setBatchCtl(null) }}>取消流水线</Button>
                    ) : (
                      <>
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-slate-100 bg-slate-50/50 text-left text-xs font-medium uppercase tracking-wider text-slate-400">
                        ...
                        <th className="px-4 py-2.5">流水线</th>
                    ...
                    <tbody>
                      {filteredDocs.map(doc => (
                        <DocumentRow key={doc.id} doc={doc} onDelete={handleDeleteDoc} onProcess={handleProcess}
                          ...
                          batchSelected={batchIds.has(doc.id)}
                          batchProgress={batchProgress.get(doc.id)}
