...
用法::
    ...
    python -m server.rag.document_processing.pipeline <name>

示例::

    python -m server.rag.document_processing.pipeline demo1
...
from server.rag.chunking.figures import extract_figures_from_md, assign_pages_to_figures
...
def step2_md_to_chunks(md_path: pathlib.Path, chunks_path: pathlib.Path, source: str):
    """Markdown → 分块 JSON。"""
    from server.rag.chunking.splitter import _parse_markdown_to_pages, split_markdown_by_page, _remove_reference_section
...
def kb_chroma_dir(kb_id: int) -> pathlib.Path:
    """KB 级别 ChromaDB 目录路径。"""
    from server.core.config import PROJECT_ROOT, OUTPUT_CHROMA_DIR, KB_CHROMA_DIR_PREFIX
    return PROJECT_ROOT / OUTPUT_CHROMA_DIR / f"{KB_CHROMA_DIR_PREFIX}{kb_id}"


def kb_collection_name(kb_id: int) -> str:
    """KB 级别 ChromaDB 集合名称。"""
    from server.core.config import KB_COLLECTION_PREFIX, COLLECTION_NAME_SUFFIX
