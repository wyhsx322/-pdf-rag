...
from server.agents.base import sse
...
async def run_workflow_endpoint(project_id: int, req: RunAgentRequest):
    ...
    from server.agents.workflow import run_workflow, ThesisState
...
async def generate_outline(project_id: int, req: GenerateOutlineRequest):
    ...
    from pathlib import Path
    sys.path.insert(0, str(Path(__file__).parent.parent.parent))
    from server.agents.outline import OutlineAgent
    ...
    async def stream():
        try:
            ...
            if req.with_literature:
                from server.agents.literature import LiteratureAgent
...
def confirm_section(project_id: int, section_id: str):
    ...
    from server.agents.section_memory import summarize_section
...
def refine_paragraph(project_id: int, section_id: str, req: RefineRequest):
    ...
    from server.agents.prompts import REFINE_SYSTEM, REFINE_USER
